...
connect(URL) ->
  ...
  [ServerVersion, <<_ThreadId:32/little, Scramble1:8/binary, 0, _Caps1:16, Charset, _Status:16, _Caps2:16,
    AuthLen, _Reserve1:10/binary, Scramble2:13/binary, Pluginz/binary>>] = binary:split(Rest1, <<0>>),
  21 = AuthLen,
  [_Plugin|_] = binary:split(Pluginz, <<0>>),
...
command(Cmd, Data, Spec) when is_list(Spec) orelse is_binary(Spec) ->
  case connect(Spec) of
    {ok, Sock} ->
      Result = command(Cmd, Data, Sock),
      close(Sock),
      Result;
    {error, E} ->
      {error, E}
...
close(#conn{socket = Sock}) ->
  close(Sock);
close(Sock) ->
  gen_tcp:close(Sock).


send_packet(#conn{socket = Sock}, Number, Bin) ->
